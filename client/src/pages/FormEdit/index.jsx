...
import { editTransRequest, fetchDataRequest } from './actions';
...
const FormEdit = ({ trans }) => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(editTransRequest({ formDataObj, id_trans }));
  };

  useEffect(() => {
    dispatch(fetchDataRequest(id_trans));
  }, []);
  ...
  useEffect(() => {
    if (trans?.data) {
      setFormData({
        ...
        des: trans?.data.des,
        facility: trans?.data.facility,
        pWeek: trans?.data.pWeek,
        pDay: trans?.data.pDay,
        categoryId: trans?.data.categoryId,
      });
    }
  }, [trans?.data]);
  return (
